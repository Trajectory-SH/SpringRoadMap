...
public class ValidationItemControllerV2 {
    ...
    //@ModelAttribute와 같이 컨트롤러가 호출되기 전에 validator도 선 처리가 가능하다.
    @InitBinder
    ...
    /**
     * 어떤 로직을 설계할 때 검증로직 -> 정상로직 순서로 설계하는 것이 관례
     */
    //@PostMapping("/add")
   ...
   // @PostMapping("/add")
    public String addItemV3(@ModelAttribute Item item, BindingResult bindingResult, RedirectAttributes redirectAttributes) {
        ...
        //item이 save가 되어야지 Id값 (Sequence)가 할당되기 때문에 -> 해당 log에서 id값은 null이다.
    ...
    //@PostMapping("/add")
        public String addItemV4(@ModelAttribute Item item, BindingResult bindingResult, RedirectAttributes redirectAttributes) {
            ...
            if (!StringUtils.hasText(item.getItemName())) {
                ...
                //내부에서 MessageCodeResolver가 실행되어 해당 파라미터 정보를 보고 총 4개의 메시지 코드를 생성해준다.
            }
        ...
        log.info("objectName_02={}", bindingResult.getObjectName());
        log.info("target_02={}", bindingResult.getTarget());
